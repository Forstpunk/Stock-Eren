...
- signal: at least one testable feature holds
...
from intraday.features import FEATURE_EXPECTATION, FEATURE_NAMES
...
def bust_rate_by_third(rows: pd.DataFrame, feature: str, scope: str) -> FeatureTable | None:
    """Cut ``feature`` into thirds by value and count busts in each. None if a third would
    be empty (too few distinct values, or too few rows)."""
    ...
    lower_bounds = [float(values.min()), edges[0], edges[1]]
    upper_bounds = [edges[0], edges[1], float(values.max())]
    masks = [values <= edges[0], (values > edges[0]) & (values <= edges[1]), values > edges[1]]

    thirds = []
    for name, lo, hi, mask in zip(THIRD_NAMES, lower_bounds, upper_bounds, masks):
        n = int(mask.sum())
        if n == 0:
            return None
...
def _finding(rows: pd.DataFrame, feature: str, split: date | None, min_half_busts: int) -> FeatureFinding:
    overall = bust_rate_by_third(rows, feature, "all")
    if overall is None:
        raise ValueError(f"{feature}: cannot cut into thirds ({rows[feature].notna().sum()} usable rows)")
    first = second = None
    ...
    holds = testable and first is not None and second is not None and (
        first.gap_pct >= MIN_GAP_PCT and second.gap_pct >= MIN_GAP_PCT
    )
    ...
    return FeatureFinding(
        ...
        testable=testable, holds=holds, statement=statement,
    )
...
def run_study(features: pd.DataFrame, config: Config) -> Study:
    ...
    findings = tuple(_finding(features, f, split, config.min_sample) for f in FEATURE_NAMES)
    base_rate = float((features["label"] == Label.BUSTED.value).mean() * 100)

    held = [f.feature for f in findings if f.holds]
    testable = [f.feature for f in findings if f.testable]
    if not testable:
        ...
        statement = (
            ...
            f"{MIN_GAP_PCT:.0f} points in both halves of the period. Still one regime; confirm on longer history."
        )
    else:
        verdict = "no_signal"
